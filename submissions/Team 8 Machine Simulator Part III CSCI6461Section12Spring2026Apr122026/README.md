...
<p>Part III in this repository completes machine-fault handling, TRAP execution flow, card-reader backed file input, and Program 2 integration.</p>

### Overall Design

### Machine Faults and TRAP

- Reserved memory locations `0..5` are protected from user-program access.
- Machine faults save the current `PC` to memory location `4`, set `MFR`, and vector through memory location `1`.
- `TRAP` saves the next `PC` to memory location `2` and vectors through the trap-table base stored at memory location `0`.
- Reset initializes:
  - `memory[0] = 020` (trap table base)
  - `memory[1] = 006` (fault handler)
  - `memory[6] = HLT`

### Program 2

- Program 2 source file path: `./programs/part3/program2/source.src`
- Program 2 machine-code files:
  - `./programs/part3/program2/output.load`
  - `./programs/part3/program2/output.lst`
- Program 2 test data:
  - `./programs/part3/program2/paragraph.txt`

If you are operating from the packaged submission folder instead of the repository root, the corresponding paths are under:

- `./Part III/programs/part3/program2/...`

The simulator includes three built-in Part III trap services used by Program 2:

- `TRAP 4`: read the paragraph from the card-reader file, preserve punctuation, print it, and build searchable metadata
- `TRAP 5`: read one uppercase query word from the keyboard queue
- `TRAP 6`: search the stored words and print `WORD SENTENCE WORD`

Program 2 now detects sentence boundaries dynamically from `.`, `!`, and `?`, so sentence lengths do not need to be fixed.
...
- The printer panel supports `Numeric` and `Text` output modes.
- The card-reader panel loads the bundled paragraph file for Program 2.

### Notes and Documentation

- Part III design notes: `./DESIGN_NOTES_PART3.md`
- Part III usage guide: `./GUI_AND_PROGRAM2_INSTRUCTIONS.md`
